...
class MakeATicket(discord.ui.View):
    ...
    async def button_callback(self, button, interaction):
        ...
        ticket_channel = await interaction.guild.fetch_channel(self.server_settings.mod_channel['server_support'])
        # staff = interaction.guild.get_role(self.server_settings.admin_roles["staff"])

        with open('tickets.log', 'r+') as ticket_log:
            ticket_num = ticket_log.read()
            if ticket_num == '':
                ticket_num = 0
                ticket_log.seek(0)
                ticket_log.write('1')
                ...
                ticket_log.seek(0)
                ticket_log.write(str(int(ticket_num)+1))
        ...
        embed.set_footer(text='To speed up resolution times, please describe your issue in detail.')
        ...
        await ticket.send(f'**{interaction.user.mention}, we have received your ticket and '
